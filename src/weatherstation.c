...
static void gameCallback(RTCDRV_TimerID_t id, void *user);
static gesture_t handleGestures(void);
...
int main(void)
{
  ...
  */
  ...
  //RTCDRV_AllocateTimer(&gameTimerId);
  ...
  while (1)
  {
    ...
    if (demoMode)
    {
      ...
      {
        offset = xoffset;
        GRAPHICS_Draw(offset, tempData, rhData, uvData, yoffset, lowBat, currentGesture);
        updateDisplay = false;
...
static gesture_t handleGestures(void)
{
  ...
  if (gestureInput != NONE)
  {
    ...
    {
    	xinc = 0;
    	//GLIB_drawString(&glibContext, "butt", 10, 22, 52, 0);
    }
    if (gestureInput == LEFT)
    {
      xinc = 0;
      //GLIB_drawString(&glibContext, ";ase", 10, 22, 52, 0);
    }
    updateDisplay = true;
...
static void enableGestureMode(void)
{
  ...
  /*
  RTCDRV_StartTimer(gameTimerId, rtcdrvTimerTypePeriodic,
                    500, gameCallback, NULL);
  */
