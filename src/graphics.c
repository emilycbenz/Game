...
static void GRAPHICS_DrawTemperatureC(int xoffset, int yoffset, int32_t tempData, gesture_t currentG);
static void GRAPHICS_DrawTemperatureF(int xoffset, int yoffset, int32_t tempData, gesture_t currentG);
...
void GRAPHICS_ShowStatus(bool si114x_status, bool si7013_status, bool removeObject, bool lowBat)
{
  ...
  {
    GLIB_drawString(&glibContext, DEMO_VERSION, 16, 64, 115, 0);
  }
  DMD_updateDisplay();
}
...
void GRAPHICS_Draw(int xoffset, int32_t tempData, uint32_t rhData,
                   uint32_t uvData, int yoffset, bool lowBat, gesture_t currentG)
{
  /* Draw background */
  ...
  GRAPHICS_DrawTemperatureF(xoffset, yoffset, tempData, currentG);
  GRAPHICS_DrawTemperatureC(xoffset, yoffset, tempData, currentG);
...
/*
  if(gTime % 2 == 0){
  GLIB_drawString(&glibContext, "timeeven ", 10, 22 - dx, 72 - dy, 0);
  }


  if(gTime % 2 == 1){
	  GLIB_drawString(&glibContext, "timeodd ", 10, 22 - dx, 72 - dy, 0);
  }
  ...
  */
  ...
  if(currentG == LEFT){
  GLIB_drawString(&glibContext, "isleft", 10, 22 - dx, 52 - dy, 0);
  }

  if(currentG == RIGHT){
	  GLIB_drawString(&glibContext, "isbutt", 10, 22 - dx, 52 - dy, 0);

  }
  if(currentG == UP){
	  GLIB_drawString(&glibContext, "isupperbutt", 15, 22 - dx, 52 - dy, 0);
  }
  if (currentG == DOWN){
	  GLIB_drawString(&glibContext, "isdownwardbutt", 15, 22 - dx, 52 - dy, 0);
  }
...
//  GLIB_drawLineV(&glibContext, 98 - dx, 91, height);
 ...
 // GLIB_drawString(&glibContext, "40", 2, 72 - dx, 15 - dy, 0);
  ...
  /*
  if(gTime % 2 == 0){
  GLIB_drawString(&glibContext, "Timeeven ", 10, 22 - dx, 100 - dy, 0);
  }


  if(gTime % 2 == 1){
	  GLIB_drawString(&glibContext, "Timeodd ", 10, 22 - dx, 100 - dy, 0);
  }
